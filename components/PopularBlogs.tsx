...
const PopularBlogs: React.FC<BlogPageProps> = ({
  ...
  return (
    <div className="bg-background section-padding ">
      <Container>
        ...
        <div
          className={`mt-5 md:mt-0 order-2 md:order-1 grid grid-cols-1 md:grid-cols-3 gap-6 `}
        >
          {mostViewed.map((blog) => (
            <Link
              key={blog._id}
            ...
            >
              ...
              <div className="w-full aspect-[12/6] relative overflow-hidden rounded-t-xl">
                <Image
                  src={
                    blog.mainImage ? urlFor(blog.mainImage).url() : NotReadyBlog
                  }
                  alt={blog.title}
                  fill
                  sizes="
