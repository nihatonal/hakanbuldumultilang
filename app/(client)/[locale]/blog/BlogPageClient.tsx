...
const BlogPageClient: React.FC<BlogPageProps> = ({
  ...
  }, [pathname, router, searchQuery, selectedCategory]);
  ...
  // 🔁 Son yazılar
  const { data: latestBlogs, mutate: mutateLatest } = useSWR(
    LATEST_BLOGS_QUERY,
    fetcher,
    {
  ...
  // 🔁 En çok okunanlar
  const { data: mostViewed, mutate: mutateMostViewed } = useSWR(
    MOST_VIEWED_QUERY,
    fetcher,
    {
  ...
  useEffect(() => {
    ...
    const sub2 = client
      .listen(LATEST_BLOGS_QUERY)
      .subscribe(() => mutateLatest());
    const sub3 = client
      .listen(MOST_VIEWED_QUERY)
      .subscribe(() => mutateMostViewed());
    ...
    <div className="bg-background py-10">
      <Container>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-0 md:gap-8 mt-4 text-primary">
          ...
          <div
            className={`mt-6 md:mt-0 order-2 md:order-1 grid grid-cols-1 md:grid-cols-2 gap-6 
                    `}
          >
          ...
          {latestBlogs.length > 0 && (
            <div className="md:hidden bg-white p-4 rounded-xl shadow-sm order-3 mt-10">
              <h4 className="font-semibold text-lg mb-4 text-primary/90">
                Son Yazılar
              </h4>
